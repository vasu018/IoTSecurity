...
ax.bar(X+width+0.03, B, width, color='cyan', edgecolor='cyan', linewidth=6, log=True, label='PolicyList2(Manual)')
ax.bar(X+2*width+0.06, C, width, color='magenta', edgecolor='magenta', linewidth=6, log=True, label='PolicyList3(Manual)')
ax.bar(X+2*width+0.15, D, width, color='yellow', edgecolor='yellow', linewidth=6, log=True, label='PolicyGroup1(Abstraction)')
ax.bar(X+3*width+0.18, E, width, color='blue', edgecolor='blue', linewidth=6, log=True, label='PolicyGroup2(Abstraction)')
ax.bar(X+4*width+0.21, F, width, color='g', edgecolor='g', linewidth=6, log=True, label='PolicyGroup3(Abstraction)')
...
plt.xticks(np.arange(len(A))+3*width+0.015, ['Smart-Home', 'Smart-Campus', 'Smart-City'], fontsize=18)
plt.ylim([1, 10000000000])
plt.grid(linestyle='dotted')
plt.legend(ncol=2, fontsize=18, loc='upper left')
plt.grid(True)
plt.legend(loc = 'best', fontsize = 18)
plt.savefig("../Results/rules-abs.pdf", bbox_inches='tight')
plt.show()
plt.show()
