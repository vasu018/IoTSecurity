...
def compose_policy(policylist,abslevels,lowestlevels,G,sideG):
...
def create_graphs():
    users = []
    for (dirpath, dirnames, filenames) in walk("policies1"):
        users.extend(filenames)
        break
    #users = ["user1","user2"]
    Graphs = {}
    abstractions = json.load(open("abstractions/abs2.json", "r"))
    for user in users:
        lowestlevels={}
        G = nx.MultiDiGraph()
        sideG = nx.MultiDiGraph()
        policylist = digestPolicies("policies1/"+user)
        set_min_level(policylist,abstractions,lowestlevels)
        compose_policy(policylist,abstractions,lowestlevels,G,sideG)
        Graphs[user] = {}
        Graphs[user]['main'] = G
        Graphs[user]['side'] = sideG
        
        print("Main Graph for "+user+":")
        
#         for n in Graphs[user]['main'].adjacency_iter():
#             if n[1]:
#                 print(n)
                
        print("Sub Graph for "+user+":")
        
#         for n in Graphs[user]['side'].adjacency_iter():
#             if n[1]:
#                 print(n)
